...
import com.xh.HostService;
import com.xh.util.ContentUtil;
...
public class AMSHook implements Callback {
	...
	public AMSHook() {
		...
		application = ContentUtil.getApplication2Method();
		...
		try {
			...
			if ("android.app.ActivityThread$CreateServiceData"
				...
				if(start_service.size()>0){
					info.name = start_service.get(0).getComponent().getClassName();
